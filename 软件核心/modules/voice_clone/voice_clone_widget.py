...
from .audio_tools import make_output_path, open_output_dir, play_audio
from .config import APP_ROOT, ENV_REPORT_FILE, OUTPUT_DIR, load_config, save_config
from .model_manager import check_model_path
from .task_worker import EnvCheckWorker, ReferenceTranscribeWorker, VoiceGenerateWorker
...
class VoiceCloneWidget(QWidget):
    ...
    def _build_tts_tab(self):
        ...
        layout.addWidget(self.tts_generate_btn, 3, 1)
        layout.addWidget(self.tts_progress, 3, 2, 1, 2)
        self._row("输出文件", self.tts_output, 4, layout)
    ...
    def _build_clone_tab(self):
        ...
        layout.addWidget(self.clone_generate_btn, 4, 1)
        layout.addWidget(self.clone_progress, 4, 2, 1, 2)
        self._row("输出文件", self.clone_output, 5, layout)
    ...
    def _build_ultimate_tab(self):
        ...
        layout.addWidget(self.ultimate_generate_btn, 8, 1)
        layout.addWidget(self.ultimate_progress, 8, 2, 1, 2)
        self._row("输出文件", self.ultimate_output, 9, layout)
        layout.addWidget(listen_btn, 9, 2)
        layout.addWidget(open_btn, 9, 3)
        layout.addWidget(tip, 10, 1, 1, 3)
    ...
    def _build_batch_tab(self):
        ...
        start_btn.clicked.connect(lambda: QMessageBox.information(self, "V1 预留", "批量生成已预留，后续会接入逐行任务队列。"))
        ...
        layout.addWidget(self.batch_counts, 3, 2)
        layout.addWidget(QLabel("任务列表"), 4, 0)
        layout.addWidget(self.batch_list, 4, 1, 1, 2)
        return page
